...
import com.dididi.pocket.core.ui.dialog.PocketAlertDialog
...
class ShopCartItemDelegate : BottomItemDelegate(),
    ...
    override fun onItemChildClick(adapter: BaseQuickAdapter<*, *>, view: View, position: Int) {
        ...
        val id = view.id
        when (id) {
            R.id.item_shopcart_shop_select -> {
                //点击选择店铺
            ...
            R.id.item_shopcart_shop_name -> getParentDelegate<PocketDelegate>().supportDelegate
                    .start(MerchantPageDelegate.getStartShop(shop.shopId))
            R.id.item_shopcart_goods_select -> {
                //点击选择商品
            ...
            R.id.item_shopcart_goods_style -> {
                //点击更改商品样式
            ...
            R.id.item_shopcart_goods_increase -> {
                //点击增加商品
            ...
            R.id.item_shopcart_goods_decrease -> {
                //点击减少商品
                var count = shop.t.goodsCount
                if (count == 1) {
            ...
            R.id.item_shopcart_goods_delete -> //点击删除商品
                if (position >= 0 && position < shopList.size) {
                    removeGoods(position, "确认删除商品吗？")
                }
            R.id.item_shopcart_goods_collect -> {
                //点击收藏商品
                content = StringBuilder()
                content.append("你收藏了")
                        .append(shop.t.goodsName)
    ...
    private fun removeGoods(position: Int, content: String) {
        val dialog = PocketAlertDialog.getInstance(context)
    ...
    private fun notifyPriceChanged() {
        ...
        delegate_shoppingCart_shopcart_all_price!!.append(BigDecimal(mTotalPrice)
