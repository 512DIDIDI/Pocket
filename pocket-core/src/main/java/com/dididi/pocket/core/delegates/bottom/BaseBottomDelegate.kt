...
abstract class BaseBottomDelegate : PocketDelegate(), View.OnClickListener {
    ...
    protected var mBottomBar: LinearLayoutCompat? = null
    ...
    override fun onBindView(savedInstanceState: Bundle?, rootView: View) {
        ...
        for (i in 0 until size) {
            ...
            if (i == mIndexDelegate) {
                item.setBackgroundColor(resources.getColor(mPressColor))
                icon.setTextColor(resources.getColor(R.color.textColorDark))
                title.setTextColor(resources.getColor(R.color.textColorDark))
            }
        }
    ...
    private fun resetColor() {
        ...
        for (i in 0 until count) {
            ...
            val icon = item.getChildAt(0) as IconicsTextView
            val title = item.getChildAt(1) as AppCompatTextView
            item.setBackgroundColor(resources.getColor(mNormalColor))
            icon.setTextColor(resources.getColor(R.color.textColorHint))
            title.setTextColor(resources.getColor(R.color.textColorHint))
        }
    }
